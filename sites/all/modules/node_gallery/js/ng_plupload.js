  ...
  var init = function () {
    ...
    self.uploader.bind('FileUploaded', function () {
      if (self.uploader.total.queued == 0) {
        window.location.href = Drupal.settings.basePath + Drupal.settings.node_gallery.url_prefix + 'node/' + self.gid + '/edit';
      }
    });
    self.uploader.bind('BeforeUpload', function () {
      if (!self.gid || self.gid < 0) {
        // create the gallery on the fly.
        var url = Drupal.settings.basePath + Drupal.settings.node_gallery.url_prefix + 'node-gallery/json/gallery/create/' + self.galleryType;
