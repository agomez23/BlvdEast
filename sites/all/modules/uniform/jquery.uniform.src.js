...
(function($) {
  $.uniform = {
    options: {
		...
		textClass: 'uniform-text',
		textAreaClass: 'uniform-textarea',
		btnClass: 'uniform-button',
    ...
    if(options.resetSelector != false){
      $(options.resetSelector).mouseup(function(){
		...
		function doSelect(elem){
			...
			spanTag.html($.trim(elem.children(":selected").text()));
       	...
       	spanTag.text($.trim(elem.children(":selected").text()));
     	})
     	.focus(function() {
		...
		function doBtn(elem){
			...
			if($(elem).is(":submit"))
				divTag.click(function(){$(elem.get(0).form).submit()});
			else
				divTag.click(function(){elem.click()});
      ...
      $el.after(filenameTag);
        ...
        $e = $(this);
  			...
  			}else if($e.is(":button") || $e.is(":submit") || $e.is(":reset")){
  				//element is a select
  				divTag = $e.parent("div");
    ...
    return this.each(function() {
			if($.selectOpacity){
				...
				if(elem.is("select")){
				  ...
				  doText(elem);
				}else if(elem.is(":button") || elem.is(":submit") || elem.is(":reset")){
				  //element is a button
				  doBtn(elem);
				}
