jQuery(function(){
	/*jQuery('body').add(document).scroll(function(){
		min = 0;
		if(jQuery('#block-ue_custom-1').prev().length > 0)
			min = jQuery("#block-ue_custom-1").prev().outerHeight()+jQuery('#block-ue_custom-1').prev().offset().top-jQuery('#block-ue_custom-1').parent().offset().top+16;
		max = jQuery('#main-inner').innerHeight()-jQuery('#block-ue_custom-1').outerHeight()-19;
		jQuery('#block-ue_custom-1').css({position:'absolute'}).stop().animate({
			top:Math.min(Math.max(jQuery(document).scrollTop()-jQuery('#sidebar-right-inner').offset().top+16,min), max)
		})}
	);
	
	jQuery('#block-ue_custom-1 .content>ul>li:gt(0) ul').hide();
	*/
	//jQuery('#block-ue_custom-1 li:first-child a[rel=form]').addClass('active');	
	
	jQuery('#block-ue_custom-1 a[rel=form]').click(function(){switchTab(jQuery(this).attr('href').replace('#', '')); return false});
	/*{
		...
		if(fieldset.length > 0)
		{
			...
			//jQuery(this).parents('#block-ue_custom-1').find('ul ul:visible').not(jQuery(this).siblings('ul')).slideToggle();
